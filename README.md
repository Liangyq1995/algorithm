...
- **纯函数优先** — 绝大多数题目为模块级函数；设计题（LRU、Trie 等）保留为独立类
- **可浏览的题目手册** — `catalog/index.html`，支持搜索、难度筛选、代码懒加载与一键复制
- **增量加题** — 使用项目 Skill `.cursor/skills/add-algorithm-problem/`，小规模登记新题，无需全量重建
...
├── paradigms/                  # 算法范式
├── strings/                    # 字符串操作与模式匹配
├── catalog_src/                # 手册 UI 源（css/js/模板）
├── catalog/                    # 离线题目手册（成品）
│   ├── index.html
│   ├── catalog.css
│   ├── catalog.js
│   └── problems-data.js
├── .cursor/skills/add-algorithm-problem/  # 增量加题 Skill
└── README.md
```
...
直接在浏览器中打开 [`catalog/index.html`](catalog/index.html)。

### 新增一道题

1. 在对应模块写好函数/类（docstring 首行：`题号. 题名`）
2. 用 Skill 脚本增量登记（勿再引入全量 meta 生成链路）：

```bash
python .cursor/skills/add-algorithm-problem/scripts/add_to_catalog.py \
  --file path/to/module.py --name function_or_class_name --info info.json
```

详情见 `.cursor/skills/add-algorithm-problem/SKILL.md`。在 Cursor 里说「加题」也会触发该 Skill。

改手册样式/交互：编辑 `catalog_src/static/`，再复制到 `catalog/`。
...
| 文件 | 内容 |
|------|------|
| `backtracking/core.py` | 回溯：组合/排列/子集、N 皇后、括号生成、单词搜索等 |
...
| 手册增量更新 | 新题通过 Skill 写入 `catalog/`，不维护独立 meta 全量生成 |
...
graph TD
    ...
    C & D & E & F & G --> H[catalog/index.html 题目手册]
```
